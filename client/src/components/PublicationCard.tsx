...
export function PublicationCard({ publication, viewMode, canEdit, busy, onOpen, onEdit, onStatus }: {
  ...
  return (
    <article className={`publication-card publication-card--${viewMode}${overdue ? ' publication-card--overdue' : ''}`} tabIndex={0} role="group" aria-label={`Відкрити публікацію «${publication.title}»`} onClick={openCard} onKeyDown={(event) => { if (event.target === event.currentTarget && ['Enter', ' '].includes(event.key)) { event.preventDefault(); onOpen(publication); } }}>
      <header>
      ...
      <div className="publication-card__meta">
        ...
        <span><span className="mini-avatar mini-avatar--accepted">{getInitials(publication.assignee.name)}</span><span><small>Відповідальний</small><strong>{publication.assignee.name}</strong></span></span>
      </div>
