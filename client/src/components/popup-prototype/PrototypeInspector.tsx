...
import { demoProducts, fieldTypeLabels, moveItem, type Device, type PrototypeDraft, type PrototypeField, type Selection } from './model';
...
function Range({ label, value, min, max, onChange, unit = 'px' }: { label: string; value: number; min: number; max: number; onChange: (value: number) => void; unit?: string }) {
  const id = useId();
  return <label className="pp-range" htmlFor={id}><span>{label}<output aria-hidden="true">{value} {unit}</output></span><input id={id} aria-label={label} type="range" min={min} max={max} value={value} onChange={(event) => onChange(Number(event.target.value))} /></label>;
}
...
function Color({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return <label className="pp-color"><span>{label}</span><div><input type="color" value={value} onChange={(event) => onChange(event.target.value)} /><code>{value.toUpperCase()}</code></div></label>;
}
...
export function PrototypeInspector({ draft, device, selection, onSelect, update }: InspectorProps) {
