...
export function ApplicationFormPlacementEditor({ form }: Props) {
  ...
  return <div className="form-placement-editor">
    ...
    <section className="tool-panel form-placement-editor__catalog">
      ...
      <div className="form-placement-product-list">
        {(catalog.data?.items || []).map((product) => {
          const parentTarget = productTarget(product);
          return <article key={product.id}>
            <header>
              {product.primaryImageUrl ? <img src={product.primaryImageUrl} alt="" loading="lazy" /> : <span className="form-placement-product-placeholder"><Icon name="productSelection" size={20} /></span>}
              <span><strong>{firstTitle(product.titles)}</strong><small>SKU: {product.sku || '—'} · {product.modifications.length} модифікацій</small></span>
              <label className="check-field"><input type="checkbox" checked={isTargetSelected(parentTarget)} onChange={() => toggleTarget(parentTarget)} /><span>Увесь товар</span></label>
            </header>
            {product.modifications.length > 0 && <div>{product.modifications.map((modification) => {
              const target = modificationTarget(product, modification);
              return <label className="check-field" key={modification.id}><input type="checkbox" checked={isTargetSelected(target)} onChange={() => toggleTarget(target)} /><span><strong>{firstTitle(modification.titles, firstTitle(product.titles))}</strong><small>SKU: {modification.sku || '—'} · {modification.availability || 'наявність не вказана'}</small></span></label>;
            })}</div>}
          </article>;
        })}
