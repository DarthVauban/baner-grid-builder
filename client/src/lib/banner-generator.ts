...
export const BANNER_STYLES = `.mt-banner-grid {
...
const BANNER_SCRIPT = `<script>
  (function () {
    ...
    function end(card) {
      ...
      return new Date(date[0], date[1] - 1, date[2], time[0], time[1], 59, 999).getTime();
    }
    ...
    function refresh() {
      Array.prototype.forEach.call(
        document.querySelectorAll(".mt-banner-card[data-mt-end-date]"),
        function (card) {
          var left = end(card) - Date.now();
          var label = card.querySelector(".mt-banner-date");
          var expired = left < 0;

          if (label) {
            if (expired) {
              label.textContent = "Акція завершена";
            } else if (left >= 86400000) {
              var days = Math.floor(left / 86400000);
              label.textContent = "До закінчення акції " + days + " " + word(days);
            } else {
