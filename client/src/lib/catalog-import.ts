...
export function buildCatalogImportWorkbook(schema: CatalogImportTemplateSchema) {
  ...
  const headers = [...schema.columns.map((column) => column.label), ...characteristicFields.map((field) => field.header)];
  const importSheet = XLSX.utils.aoa_to_sheet([headers]);
  importSheet['!cols'] = [
    ...schema.columns.map((column) => ({ wch: Math.min(42, Math.max(12, column.width || column.label.length + 4)) })),
  ...
  const exampleCore = schema.columns.map((column) => {
    ...
    if (column.key === 'groupLabel') return 'iPhone 13';
    if (column.key === 'groupMain') return 'Так';
  ...
  const exampleSheet = XLSX.utils.aoa_to_sheet([headers, [...exampleCore, ...exampleCharacteristics]]);
  ...
  const characteristicRows: Array<Array<string | number>> = [
    ['Шаблон', 'Характеристика', 'Ключ', 'Тип', 'Одиниця', 'Обовʼязкова', 'Параметр модифікації', 'Допустимі значення', 'Заголовок XLSX'],
    ...schema.templates.flatMap((template) => template.fields.map((field) => [
      ...
      field.isModifier ? 'Так' : 'Ні',
      field.options.join('; '),
      field.header
    ]))
  ];
  const characteristicsSheet = XLSX.utils.aoa_to_sheet(characteristicRows);
  characteristicsSheet['!cols'] = [{ wch: 28 }, { wch: 28 }, { wch: 22 }, { wch: 24 }, { wch: 12 }, { wch: 16 }, { wch: 24 }, { wch: 42 }, { wch: 72 }];
  characteristicsSheet['!autofilter'] = { ref: `A1:I${characteristicRows.length}` };
  ...
  const helpRows: Array<Array<string>> = [
    ...
    ['Модифікації', 'Однакова назва групи обʼєднує товари. У межах групи позначте не більше однієї основної модифікації.']
  ];
  ...
  workbook.Props = {
    ...
    Subject: 'Імпорт товарів, характеристик і модифікацій',
    Author: 'MT Workspace'
  };
  return workbook;
}
