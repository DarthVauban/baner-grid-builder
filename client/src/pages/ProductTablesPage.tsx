...
export function ProductTablesPage() {
  ...
  function toggleStatus(field: 'completed' | 'uploaded') {
    if (!activeSheet) return;
    const visibility = field === 'completed' ? 'showCompletedStatus' : 'showUploadedStatus';
    const showing = activeSheet[visibility];
    if (showing && activeSheet.rows.some((row) => row[field]) && !window.confirm('У колонці є позначені товари. Прибрати колонку та очистити позначки?')) return;
      ...
      {view === 'library' ? (
        ...
        <section className="tool-panel sheet-editor">
          <div className="sheet-workspace">
            ...
            <div>
              <button className="button button--secondary button--small" onClick={() => reset(false)}>Нова</button>
          ...
          <div className="sheet-toolbar">
            ...
            <div className="sheet-toolbar__actions">
              <button className={activeSheet?.showCompletedStatus ? 'active' : ''} onClick={() => toggleStatus('completed')}><Icon name={activeSheet?.showCompletedStatus ? 'remove' : 'add'} size={14} /> Заповнено</button>
              <button className={activeSheet?.showUploadedStatus ? 'active' : ''} onClick={() => toggleStatus('uploaded')}><Icon name={activeSheet?.showUploadedStatus ? 'remove' : 'add'} size={14} /> Вивантажено</button>
