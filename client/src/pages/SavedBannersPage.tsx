...
export function SavedBannersPage({ embedded = false }: { embedded?: boolean }) {
  ...
  async function remove(id: string, name: string) {
    ...
    try { await api.banners.remove(id); await queryClient.invalidateQueries({ queryKey: ['saved-banners'] }); setMessage('Банер видалено.'); }
    catch (error) { setMessage(error instanceof Error ? error.message : 'Не вдалося видалити банер.'); }
  }
