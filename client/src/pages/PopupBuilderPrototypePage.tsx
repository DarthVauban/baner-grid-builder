...
function PrototypeStudio({ kind, storageKey }: { kind: PrototypeKind; storageKey: string }) {
  ...
  return <main className={`pp-studio ${fullscreen ? 'is-focus' : ''} is-panel-${mobilePanel}`}>
    <header className="pp-studio-header">
      ...
      <nav className="pp-kind-switch" aria-label="Прототип конструктора"><Link to="/tools/popup-banners/builder"><Icon name="add" size={17} />Блоковий</Link><Link to="/tools/popup-banners/prototypes/product" aria-current={!lead ? 'page' : undefined}><Icon name="productCard" size={17} />Товарний банер</Link><Link to="/tools/popup-banners/prototypes/lead-form" aria-current={lead ? 'page' : undefined}><Icon name="formBuilder" size={17} />Форма за промокод</Link></nav>
