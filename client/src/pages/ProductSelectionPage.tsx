    ...
    <div className="product-selection-workspace">
      ...
      <main className="product-selection-editor">
        <header className="product-selection-editor__header">
          ...
          <div>{!creating && <button className="icon-button icon-button--danger" type="button" onClick={() => void remove()} aria-label="Видалити вибірку"><Icon name="delete" size={18} /></button>}<button className="button button--primary button--small" type="button" onClick={() => void save()} disabled={saving || !draft.name.trim() || !draft.items.length || !catalog.data?.integration?.configured}><Icon name="save" size={16} /> {saving ? 'Зберігаємо…' : 'Зберегти'}</button></div>
        </header>
        ...
        <div className="product-selection-editor__body">
          {tab === 'appearance' && <div className="product-selection-appearance-layout">
            <section className="product-selection-section">
              <header><span><Icon name="edit" size={18} /></span><div><h2>Вигляд товарного блоку</h2><p>Задайте текст кнопки та кількість карток для кожного екрана.</p></div></header>
