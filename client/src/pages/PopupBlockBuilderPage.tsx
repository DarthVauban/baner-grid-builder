...
export function BlockStudio({ storageKey, live }: { storageKey: string; live?: LiveBlockStudio }) {
  ...
  function select(id: string) { setSettingsOpen(false); setSelection(id); setMobilePanel('properties'); }
  ...
  return <main className={'pp-studio pb-studio ' + (fullscreen ? 'is-focus ' : '') + 'is-panel-' + mobilePanel}>
    <header className="pp-studio-header" inert={live?.busy || undefined}><Link className="pp-back" to="/tools/popup-banners" onClick={live ? event => { event.preventDefault(); live.onLeave(); } : undefined} aria-label="До попап-банерів"><Icon name="arrowLeft" size={20} /></Link><div className="pp-studio-identity"><div><span>POPUP STUDIO</span><b>БЛОКОВИЙ</b></div><input aria-label="Назва макета" maxLength={160} value={draft.name} onChange={(event) => change((current) => ({ ...current, name: event.target.value }))} /></div><button type="button" className="pb-template-button" aria-expanded={templates} onClick={() => setTemplates((value) => !value)}>Шаблони <span>⌄</span></button><div className="pp-header-actions">{!live && <span className={'pp-save-state ' + (storageError ? 'is-error' : '')} role="status"><i />{storageError ? 'Помилка збереження' : 'Збережено в браузері'}</span>}{live && <button type="button" aria-pressed={settingsOpen} onClick={() => { setSettingsOpen(value => !value); setMobilePanel('properties'); }}>Умови показу</button>}<div className="pp-history"><button type="button" aria-label="Скасувати зміну" disabled={!canUndo} onClick={undo}><Icon name="undo" size={18} /></button><button type="button" aria-label="Повторити зміну" disabled={!canRedo} onClick={redo}><span className="pp-redo-icon"><Icon name="undo" size={18} /></span></button></div><button type="button" className="pb-import" onClick={() => upload.current?.click()}>Імпорт JSON</button><input ref={upload} type="file" accept="application/json,.json" aria-label="Імпорт макета" hidden onChange={(event) => void importDraft(event.target.files?.[0])} /><button type="button" className="pp-primary" onClick={exportDraft}><Icon name="download" size={16} />Експорт JSON</button>{live?.controls(draft)}</div></header>
    ...
    <div className="pp-workbench" inert={live?.busy || undefined}>
      ...
      <aside className="pp-inspector" aria-label="Властивості блока">{live && settingsOpen ? live.settings : <BlockInspector live={Boolean(live)}
        ...
        productBinding={live ? <div className="pb-product-binding"><p className="pb-help">{productSource ? `${productOwner === draft.root ? 'Товар банера' : 'Товар блока «' + productOwner.name + '»'}: ${boundProduct ? boundProduct.title + ' · ' + boundProduct.sku : 'очікуємо дані каталогу'}` : 'Товар не обрано. Прив’яжіть його, щоб цей елемент отримував дані каталогу.'}</p><button type="button" onClick={() => select(productOwner.id)}>{productSource ? 'Змінити товар' : 'Обрати товар'}</button></div> : undefined}
