...
export function BlockStudio({ storageKey, live }: { storageKey: string; live?: LiveBlockStudio }) {
  ...
  return <main className={'pp-studio pb-studio ' + (fullscreen ? 'is-focus ' : '') + 'is-panel-' + mobilePanel}>
    ...
    <div className="pp-workbench" inert={live?.busy || undefined}>
      ...
      <aside className="pp-inspector" aria-label="Властивості блока">{live && settingsOpen ? live.settings : <BlockInspector live={Boolean(live)} productPicker={live && selected.node.type === 'product' ? <CatalogBlockPicker node={selected.node} onChange={node => change(current => ({ ...current, root: patchBlock(current.root, node.id, () => node) }))} /> : undefined} key={selected.node.id} node={selected.node} device={device} onChange={(node) => change((current) => ({ ...current, root: patchBlock(current.root, node.id, () => node) }))} />}</aside>
