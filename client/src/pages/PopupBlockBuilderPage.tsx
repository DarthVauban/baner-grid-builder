...
function BlockStudio({ storageKey }: { storageKey: string }) {
  const { document: draft, update, undo, redo, canUndo, canRedo, storageError } = useBlockDocument(storageKey);
  ...
  const viewport = usePrototypeViewport(bannerWidth + 64, device + ':' + testing);
  ...
  function select(id: string) { setSelection(id); setMobilePanel('properties'); }
  function change(changeDraft: (value: BlockDocument) => BlockDocument) { perform(() => update(changeDraft)); }
  function add(type: BlockType, direction: 'row' | 'column' = 'column') { perform(() => { const block = makeBlock(type, direction); update((current) => insertBlock(current, insertionParent.id, block)); setSelection(block.id); }); }
  ...
  return <main className={'pp-studio pb-studio ' + (fullscreen ? 'is-focus ' : '') + 'is-panel-' + mobilePanel}>
    <header className="pp-studio-header"><Link className="pp-back" to="/tools/popup-banners" aria-label="До попап-банерів"><Icon name="arrowLeft" size={20} /></Link><div className="pp-studio-identity"><div><span>POPUP STUDIO</span><b>БЛОКОВИЙ</b></div><input aria-label="Назва макета" maxLength={160} value={draft.name} onChange={(event) => change((current) => ({ ...current, name: event.target.value }))} /></div><button type="button" className="pb-template-button" aria-expanded={templates} onClick={() => setTemplates((value) => !value)}>Шаблони <span>⌄</span></button><div className="pp-header-actions"><span className={'pp-save-state ' + (storageError ? 'is-error' : '')} role="status"><i />{storageError ? 'Помилка збереження' : 'Збережено в браузері'}</span><div className="pp-history"><button type="button" aria-label="Скасувати зміну" disabled={!canUndo} onClick={undo}><Icon name="undo" size={18} /></button><button type="button" aria-label="Повторити зміну" disabled={!canRedo} onClick={redo}><span className="pp-redo-icon"><Icon name="undo" size={18} /></span></button></div><button type="button" className="pb-import" onClick={() => upload.current?.click()}>Імпорт JSON</button><input ref={upload} type="file" accept="application/json,.json" aria-label="Імпорт макета" hidden onChange={(event) => void importDraft(event.target.files?.[0])} /><button type="button" className="pp-primary" onClick={exportDraft}><Icon name="download" size={16} />Експорт JSON</button></div></header>
    <div className="pp-prototype-note"><Icon name="edit" size={14} /><span>Блоковий конструктор · локальний макет · демодані в тестуванні · публікацію на сайт ще не підключено</span></div>
    {templates && <div className="pb-templates" role="group" aria-label="Шаблони банерів"><p>Почни з готової композиції або чистого аркуша. Заміну можна скасувати.</p>{(Object.keys(templateLabels) as TemplateName[]).map((name) => <button key={name} type="button" onClick={() => { const next = createTemplate(name); change(() => next); setSelection(next.root.id); setTemplates(false); setRestart((value) => value + 1); }}>{templateLabels[name]}</button>)}</div>}
    ...
    <div className="pp-workbench">
      <aside className="pp-structure" aria-label="Структура банера"><div className="pb-left-tabs"><button type="button" aria-pressed={leftTab === 'tree'} onClick={() => setLeftTab('tree')}>Шари <small>{entries.length}</small></button><button type="button" aria-pressed={leftTab === 'add'} onClick={() => setLeftTab('add')}><Icon name="add" size={15} />Додати</button></div><div className="pp-structure-scroll">
        {leftTab === 'tree' ? <><BlockTree root={draft.root} selectedId={selected.node.id} device={device} onSelect={select} onDrop={drop} /><button type="button" className="pb-add-link" onClick={() => setLeftTab('add')}>＋ Додати блок</button></> : <div className="pb-palette"><small>ДОДАТИ ВСЕРЕДИНУ</small><strong>{insertionParent.name}</strong><div className="pb-palette-layout"><button type="button" aria-label="Горизонтальний блок" onClick={() => add('container', 'row')}><span>▯ ▯</span>Горизонтальний блок</button><button type="button" aria-label="Вертикальний блок" onClick={() => add('container', 'column')}><span>▱<br />▱</span>Вертикальний блок</button></div><small>ЕЛЕМЕНТИ</small><div className="pb-palette-elements">{(['text', 'image', 'button', 'divider', 'spacer', 'coupon', 'countdown', 'form', 'field', 'product'] as BlockType[]).map((type) => <button key={type} type="button" aria-label={blockLabels[type]} onClick={() => add(type)}><span>{({ text: 'T', image: '▧', button: '↗', divider: '―', spacer: '↧', coupon: '%', countdown: '◷', form: '▤', field: '▭', product: '◇' } as Record<string, string>)[type]}</span>{blockLabels[type]}</button>)}</div><p className="pb-help">Обери контейнер на полотні або в шарах. Новий елемент потрапить усередину нього. Поля додаються лише у форму.</p></div>}
