...
export function FormButtonsPage() {
  ...
  return <div className="forms-builder-page form-buttons-page">
    ...
    {!forms.isLoading && !forms.isError && simpleForms.length === 0 && <section className="tool-panel task-list-state">
      <span className="task-list-state__icon"><Icon name="formBuilder" size={28} /></span>
      <h2>Спочатку створіть просту форму</h2>
      <p>Кнопка може відкривати будь-яку просту форму з бібліотеки. Покрокові форми мають окремий сценарій роботи.</p>
      <Link className="button button--primary" to="/tools/forms">Перейти до конструктора форм</Link>
    </section>}
    {simpleForms.length > 0 && <ApplicationFormPlacementEditor forms={simpleForms} />}
  </div>;
}
