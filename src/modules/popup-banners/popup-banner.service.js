...
function normalizeCampaignType(value, targeting = {}) {
  if (object(targeting).mode === 'out_of_stock' && !['product_promo', 'exit_offer'].includes(value)) return 'out_of_stock_recommendations';
  if (['message', 'out_of_stock_recommendations', 'product_promo', 'exit_offer'].includes(value)) return value;
  return 'message';
}
...
async function savePopupCampaign(existingId, input, actorUserId) {
  ...
  try {
    ...
    if (campaignType === 'exit_offer') {
      behavior.trigger = 'exit_intent';
      behavior.requireAcknowledgement = false;
    }
    ...
    const promoResolution = await resolvePromoItems(input.promoItems || [], connection.id, connection.generation, client);
    ...
    if (['product_promo', 'exit_offer'].includes(campaignType) && promoResolution.items.length === 0) {
      throw new AppError(422, 'POPUP_PROMO_PRODUCTS_EMPTY', campaignType === 'exit_offer'
        ? 'Додайте хоча б один товар до Exit offer.'
        : 'Додайте хоча б один товар до промобанера.', {
...
export async function resolvePopupCampaign({ pageUrl: rawPageUrl, article = '', stockState = 'unknown', requestOrigin = '' }) {
  ...
  for (const campaign of campaigns.rows) {
    ...
    const promoProducts = ['product_promo', 'exit_offer'].includes(campaignType)
      ...
      : [];
    if (['product_promo', 'exit_offer'].includes(campaignType) && promoProducts.length === 0) continue;
    return {
      campaign: {
...
export function popupEmbedScript(origin) {
  ...
  function render(payload, productArticle) {
    ...
    const isExitOffer = campaign.type === 'exit_offer';
    ...
    style.textContent += '.recommendation-image{background:#fff}.card.is-exit-offer{overflow:hidden}.card.is-exit-offer:before{position:absolute;z-index:1;top:0;right:0;left:0;height:5px;background:var(--accent);content:""}.card.is-exit-offer .recommendations{grid-template-columns:repeat(2,minmax(0,1fr))}.card.is-exit-offer .recommendation-image{aspect-ratio:1}.card.is-exit-offer .recommendation-buy{border-radius:var(--button-radius,12px)}@media(max-width:600px){.card.is-exit-offer .recommendations{grid-template-columns:1fr}}';
...
@media(max-width:360px),(max-height:560px){.card.is-product-promo:not(.format-notification) .body{display:none}.card.is-product-promo:not(.format-notification) .recommendation{grid-template-columns:96px minmax(0,1fr)}.card.is-product-promo:not(.format-notification) .recommendation-image{height:96px}.card.is-product-promo.format-notification .recommendation{grid-template-columns:48px minmax(0,1fr) auto}.card.is-product-promo.format-notification .recommendation-media,.card.is-product-promo.format-notification .recommendation-image{width:48px;height:48px}}\`;
    ...
    card.className = isProductPromo
      ...
      : campaign.mode === 'out_of_stock' ? 'card is-recommendations' : isExitOffer ? 'card is-recommendations is-exit-offer' : 'card';
    card.tabIndex = -1;
    ...
    } else card.setAttribute('aria-label', isProductPromo ? 'Товарний промобанер' : isExitOffer ? 'Exit offer' : 'Інформаційний попап');
    ...
    if (campaign.mode === 'out_of_stock' || isProductPromo || isExitOffer) {
      const recommendations = document.createElement('div'); recommendations.className = 'recommendations';
      const recommendationEntries = [];
      for (const recommendation of ((isProductPromo || isExitOffer) ? payload.products : payload.recommendations) || []) {
