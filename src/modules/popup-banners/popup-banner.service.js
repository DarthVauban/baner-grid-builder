...
function normalizeStyles(value) {
  ...
  const primaryButtonBackgroundColor = color(source.primaryButtonBackgroundColor, accentColor);
  const primaryButtonTextColor = color(source.primaryButtonTextColor, defaultStyles.primaryButtonTextColor);
  return {
    ...
    promoCopyButtonBackgroundColor: color(source.promoCopyButtonBackgroundColor, primaryButtonBackgroundColor),
    promoCopyButtonTextColor: color(source.promoCopyButtonTextColor, primaryButtonTextColor),
    promoDiscountTextColor: color(source.promoDiscountTextColor, textColor),
    promoCodeBackgroundColor: color(source.promoCodeBackgroundColor, accentColor),
    promoCodeBackgroundOpacity: Math.min(100, Math.max(0, Number.isFinite(Number(source.promoCodeBackgroundOpacity))
      ? Number(source.promoCodeBackgroundOpacity) : defaultStyles.promoCodeBackgroundOpacity)),
...
async function loadCampaignRow(id, db = { query }) {
  const result = await db.query(
    `SELECT campaign.*, connection.store_domain
     ...
     WHERE campaign.id = $1 AND campaign.campaign_type <> 'block'`,
    [id]
  );
...
export async function listPopupCampaigns() {
  const result = await query(
    `SELECT campaign.*, connection.store_domain
     FROM popup_banner_campaigns AS campaign
     LEFT JOIN search_horoshop_connections AS connection ON connection.id = campaign.connection_id
     WHERE campaign.campaign_type <> 'block'
    ...
    if (id) {
      ...
      const updated = await client.query(
        `UPDATE popup_banner_campaigns
         ...
         WHERE id = $1 AND campaign_type <> 'block' RETURNING id`,
...
export async function setPopupCampaignStatus(id, status, actorUserId) {
  ...
  try {
    ...
    const campaign = await client.query(
      "SELECT * FROM popup_banner_campaigns WHERE id = $1 AND campaign_type <> 'block' FOR UPDATE",
      [id]
    );
...
export async function deletePopupCampaign(id) {
  const result = await query("DELETE FROM popup_banner_campaigns WHERE id = $1 AND campaign_type <> 'block' RETURNING id", [id]);
  if (!result.rows[0]) throw new AppError(404, 'POPUP_CAMPAIGN_NOT_FOUND', 'Попап-кампанію не знайдено.');
}
...
export function popupEmbedScript(origin) {
  ...
  function render(payload, productArticle) {
    ...
    style.textContent += '.promo-code-offer{border-color:color-mix(in srgb,var(--promo-code-bg) 24%,transparent);background:color-mix(in srgb,var(--promo-code-bg) var(--promo-code-bg-opacity),transparent)}.promo-code-value{color:var(--promo-discount-text)}.promo-code-copy{border-color:var(--promo-copy-bg);color:var(--promo-copy-text);background:var(--promo-copy-bg)}';
...
@media(max-width:360px),(max-height:560px){.card.is-product-promo:not(.format-notification) .body{display:none}.card.is-product-promo:not(.format-notification) .recommendation{grid-template-columns:96px minmax(0,1fr)}.card.is-product-promo:not(.format-notification) .recommendation-image{height:96px}.card.is-product-promo.format-notification .recommendation{grid-template-columns:48px minmax(0,1fr) auto}.card.is-product-promo.format-notification .recommendation-media,.card.is-product-promo.format-notification .recommendation-image{width:48px;height:48px}}\`;
    ...
    backdrop.style.setProperty('--promo-copy-bg', campaign.styles.promoCopyButtonBackgroundColor);
    backdrop.style.setProperty('--promo-copy-text', campaign.styles.promoCopyButtonTextColor);
    backdrop.style.setProperty('--promo-discount-text', campaign.styles.promoDiscountTextColor);
    backdrop.style.setProperty('--promo-code-bg', campaign.styles.promoCodeBackgroundColor);
    backdrop.style.setProperty('--promo-code-bg-opacity', campaign.styles.promoCodeBackgroundOpacity + '%');
