...
Кампанія `exit_offer` використовує окремий тригер `exit_intent` і блокувальний modal-контракт.
Вона не показується за звичайною затримкою: `delayMs` визначає лише час, після якого розпізнавання
наміру виходу стає активним.

Exit offer є товарною пропозицією, тому перед збереженням до нього потрібно прикріпити хоча б
один видимий товар або модифікацію в наявності із синхронізованого каталогу Хорошопа. На вітрину
передаються лише актуальні доступні позиції з фото, посиланням і `buyId`; якщо після фільтрації
не лишилося жодного товару, кампанія не показується. Кнопка товарної картки додає позицію через
той самий desktop/mobile адаптер штатного кошика Хорошопа, що й товарний промобанер.
...
- кнопка «Назад» та History API не перехоплюються: exit offer не додає фіктивних history entries
...
Regression-тест `exit_offer` обов'язково перевіряє, що попап відсутній до сигналу, окремо
емулює desktop mouse-exit і mobile touch-exit із mobile User-Agent + viewport та підтверджує
відсутність показу від звичайного руху курсора або недостатнього mobile scroll depth. API-тест
також перевіряє збереження прикріпленого товару, а runtime-тест — його картку всередині попапа.
