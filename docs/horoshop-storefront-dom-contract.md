...
- формати `notification` і `compact` приховують надзаголовок та опис кампанії. Заголовок
  кампанії показується в них лише за явним параметром `showPromoTitle`; назва самого товару
  залишається видимою завжди. Інші формати показують налаштований текст кампанії;
