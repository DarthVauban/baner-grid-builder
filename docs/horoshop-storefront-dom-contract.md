...
Розмір QR-коду та шрифт кнопки налаштовуються незалежно для desktop і mobile. Допустимий розмір
QR — ціле число `160–320 px`, шрифт кнопки — `12–24 px`, спільне скруглення кнопки — `0–32 px`.
Workspace показує ці межі біля кожного поля та не надсилає чернетку або публікацію з невалідним
значенням. Для конфігурацій, створених до появи mobile-полів, mobile успадковує відповідні desktop
значення. Публічний payload містить окремі PNG для потрібної щільності кожної поверхні.
...
- формат `notification` приймає лише необов’язкові заголовок та основний текст; порожні поля не
  створюють плейсхолдерів або порожніх відступів, а надзаголовок і загальне зображення кампанії
  в цьому форматі ігноруються. Товарний ряд має фіксовані grid-зони `media`, `product-title`,
  `price` і `buy`, тому тексти кампанії не можуть переставити фото, ціну чи кнопку;
- у форматі `compact` параметр `showPromoTitle` керує лише заголовком кампанії і не впливає на
  основний текст або назву товару. Інші формати показують налаштований текст кампанії;
...
- якщо вибрано кілька товарів, embed по черзі показує одну картку, а нижня часова лінія відображає
  прогрес до наступного перемикання відповідно до `rotationSeconds`. Колір заповнення і підкладки
  задаються незалежно через `timelineColor` та `timelineTrackColor`. Наведення курсора на банер
  призупиняє і таймер, і анімацію лінії зі збереженням залишку часу; вихід курсора продовжує той
  самий відлік незалежно від того, який елемент банера залишається у фокусі;
- за кількох товарів між контентом і часовою лінією відображається компактна навігація
  «попередній / номер поточного / наступний». Ручне перемикання синхронно змінює товар, ціль
  фонового посилання та починає для нового товару повний відлік `rotationSeconds`;
- радіус кнопки задається через `buttonBorderRadius` і однаково застосовується у прев’ю та
  публічному embed-коді;
...
- умови показу: затримка після завантаження, відсоток прокручування, бездіяльність або
  намір покупця вийти. Умова «Намір вийти» недоступна для товарного промобанера;
...
## Умова показу «Намір вийти»

`exit_intent` — це умова показу, а не окремий тип банера. Вона застосовується до звичайного
інформаційного попапа і має бути доступна новим банерним форматам, які не мають власної
несумісної логіки. Вона чітко заборонена для:

- `product_promo` — товарного промобанера;
- `out_of_stock_recommendations` — альтернатив відсутнього товару.

При `exit_intent` банер не показується за звичайною затримкою: `delayMs` визначає лише час,
після якого розпізнавання наміру виходу стає активним. Контент, макет, кнопки та правила цілі
не змінюються: умова керує лише моментом показу.

Desktop і mobile мають незалежні сигнали:

- desktop: курсор залишає viewport через верхню межу (`mouseout` без `relatedTarget`,
  `clientY <= 10`) після завершення періоду активації;
- mobile: після того, як покупець прокрутив сторінку щонайменше на 140 px, швидкий жест униз
  повертає viewport до верхньої ділянки (`scrollY <= 80`), або покупець повертається на вкладку,
  яка була прихована щонайменше 600 мс;
- mobile-контракт вимагає одночасно mobile User-Agent і viewport до 600 px. Сам viewport не є
  доказом мобільної вітрини;
- кнопка «Назад» та History API не перехоплюються: умова не додає фіктивних history entries
  і не змушує покупця повторно виходити зі сторінки;
- усі listeners і таймер активації видаляються під час показу, зміни URL або повторного
  `evaluate()`, тому один намір не створює дублікати попапа;
- повторні покази, device targeting і розклад застосовуються тим самим механізмом, що й до інших
  кампаній. Рекомендоване початкове обмеження — один показ за сесію.

Regression-тест `exit_intent` обов'язково перевіряє, що попап відсутній до сигналу, окремо
емулює desktop mouse-exit і mobile touch-exit із mobile User-Agent + viewport та підтверджує
відсутність показу від звичайного руху курсора або недостатнього mobile scroll depth. API-тест також
перевіряє заборону цієї умови для `product_promo` і `out_of_stock_recommendations`.
