...
- формат задається пресетом: `notification` — компактне горизонтальне сповіщення з одним
  товаром і ротацією, `compact` — одна картка, `standard` — дві, `wide` — три, `custom` —
  ручна ширина. Пресет `notification` наслідує патерн ненав'язливого sales-notification
  віджета, але не залежить від стороннього сервісу;
